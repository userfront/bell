...
describe("Bell v2", () => {
  describe("v2()", () => {
    ...
    it("forces https in redirect_uri when set in options", async (flags) => {
      ...
      expect(res2.headers.location).to.contain(
        "https://localhost:8080/login?code=1&state="
      );
    ...
    it("uses location setting in redirect_uri when set in options", async (flags) => {
      ...
      const res2 = await mock.server.inject(res1.headers.location);
      expect(res2.headers.location).to.contain(
        "https://differenthost:8888/login?code=1&state="
      );
    ...
    it("uses location setting (function) in redirect_uri when set in options", async (flags) => {
      ...
      server.auth.strategy("custom", "bell", {
        ...
        location: (request) =>
          "https://differenthost:8888" +
          request.path.replace(/(\/again)?$/, "/again"),
      });
    ...
    it("authenticates an endpoint via oauth with plain PKCE", async (flags) => {
      ...
      const res2 = await mock.server.inject(res1.headers.location);
      expect(res2.headers.location).to.contain(
        "http://localhost:8080/login?code=1&state="
      );
    ...
    it("errors on failed profile request", async (flags) => {
      ...
      Mock.override("https://graph.facebook.com/v9.0/me", null);
    ...
    it("errors on errored profile request", async (flags) => {
      ...
      Mock.override("https://graph.facebook.com/v9.0/me", Boom.badRequest());
    ...
    it("errors on invalid profile request", async (flags) => {
      ...
      Mock.override("https://graph.facebook.com/v9.0/me", "{c");
    ...
    it("errors on rejected query parameter", async (flags) => {
      ...
      const res2 = await server.inject(
        '/login?error=access_denied&error_description="rejection"'
      );
      ...
      const strategyOptions = {
        ...
        postAuthorizationHook: async (request, settings) => {
          ...
          if (
            ...
            const tenant = await mockDbLookup(
              request.state[settings.cookie].tenant_id
            );
            settings.clientId = tenant.googlePublicKey;
  ...
  describe("concurrency", () => {
    it("it authenticates 6 requests with same provider", async (flags) => {
      ...
      for (const requestStates of results) {
        expect(requestStates[0]).to.equal(requestStates[1]);
      }
      ...
      async (flags) => {
        ...
        function runOAuthFlow(n) {
          return new Promise(async (resolve) => {
            ...
            const mockServer =
              provider === "github" ? githubMock.server : googleMock.server;
            ...
            const res2 = await delayedRequest(
              mockServer,
    ...
    it(
      "it authenticates 6 requests, all using different providers",
      { plan: 12 },
      async (flags) => {
        const server = Hapi.server({
          host: "localhost",
        ...
        const auth0Provider = Bell.providers.auth0({
    ...
    it(
      "it authenticates 100 requests with 6 different providers",
      { plan: 200 },
      async (flags) => {
        const server = Hapi.server({
          host: "localhost",
        ...
        const results = await Promise.all(arrayOf100.map(runOAuthFlow));
        ...
        function runOAuthFlow(n) {
          if (currentProviderIndex > 5) {
            currentProviderIndex = 0;
          }

          return new Promise(async (resolve) => {
            // Alternate between each provider based on `n`
            const providerMockObj = providerMocks[currentProviderIndex];
            const provider = providerMockObj.name;
            const mockServer = providerMockObj.mock.server;

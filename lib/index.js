...
internals.provider = Joi.object({
  ...
  otherwise: Joi.object({
    scope: Joi.alternatives(Joi.array().items(Joi.string()), Joi.func()),
...
internals.schema = Joi.object({
  ...
  clientSecret: Joi.alternatives()
    ...
    .required(),
  preAuthorizationHook: Joi.func(), // OAuth.v2 only
  postAuthorizationHook: Joi.func(), // OAuth.v2 only
  cookie: Joi.string(),
  ...
  scope: Joi.alternatives(Joi.array().items(Joi.string()), Joi.func()).when(
    "provider.protocol",
    {
      is: "oauth2",
  ...
  location: Joi.alternatives(Joi.func().maxArity(1), Joi.string()).default(
    false
  ),
...
internals.implementation = function (server, options) {
  ...
  if (typeof settings.provider === "object") {
    ...
    settings.provider = Providers[settings.provider].call(
      null,
      settings.config
    );
  }
  ...
  if (settings.provider.protocol === "oauth") {
    authenticate = OAuth.v1(settings);
  } else {
    authenticate = OAuth.v2(settings);
  }
  ...
  return {
    authenticate: async function (request, h) {
      ...
      return h.authenticated({
        credentials: Hoek.applyToDefaults(credentials, result),
      });
    },
  };
};

...
internals.schema = Joi.object({
  ...
  identityServiceProfile: Joi.boolean()
    .optional()
    .when("extendedProfile", { is: false, then: Joi.invalid(true) }),
});
...
exports = module.exports = function (options) {
  const combinedSettings = Hoek.applyToDefaults(
    internals.defaults,
    options || {}
  );
  ...
  return {
    ...
    profile: async function (credentials, params, get) {
      ...
      const profile = await get(
        settings.identityServiceProfile
          ? params.id
          : `${settings.uri}/services/oauth2/userinfo`
      );

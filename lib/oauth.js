...
/**
 * v1 does not support preAuthorizationHook & postAuthorizationHook
 */
  ...
  return async function (request, h) {
    ...
    if (protocol !== 'https' && settings.isSecure) {
      return h.unauthenticated(
        Boom.internal('Invalid setting  - isSecure must be set to false for non-https server'),
        { credentials }
      )
    }
    ...
    if (!request.query.oauth_verifier) {
      return h.unauthenticated(
        Boom.internal('Missing verifier parameter in ' + name + ' authorization response'),
        { credentials }
      )
    }
    ...
    if (request.query.oauth_token !== state.token) {
      return h.unauthenticated(Boom.internal(name + ' authorized request token mismatch'), {
        credentials,
      })
    }
  ...
  return async function (request, h) {
    ...
    if (request.query.error === 'access_denied' || request.query.denied) {
      return h.unauthenticated(
        Boom.internal(
          `App rejected: ${
            request.query.error_description || request.query.denied || 'No information provided'
          }`
        ),
        { credentials }
      )
    ...
    // 1. Sign-in Initialization (redirect user to provider)

    if (!request.query.code) {
      return createAuthorizationRedirect(request, settings, credentials, protocol, cookie, h)
    }

    // 2.  - Consent has been given. Steps:
    // 2.1 - Handle callback
    // 2.2 - Obtain token
    // 2.3 - Obtain user profile (unless skipProfile = true in strategy options)
    ...
    if (
      state.nonce !== requestState.substr(0, Math.min(requestState.length, internals.nonceLength))
    ) {
      return h.unauthenticated(Boom.internal('Incorrect ' + name + ' state parameter'), {
        credentials,
      })
    }

    if (settings.postAuthorizationHook) {
      await settings.postAuthorizationHook(request, settings)
    }
    ...
    if (!settings.provider.useParamsAuth) {
      requestOptions.headers.Authorization =
        'Basic ' +
        Buffer.from(settings.clientId + ':' + settings.clientSecret, 'utf8').toString('base64')
    }
    ...
    try {
      ...
      return h.unauthenticated(Boom.internal('Failed obtaining ' + name + ' access token', err), {
        credentials,
      })
    }
    ...
    try {
      ...
      return h.unauthenticated(
        Boom.internal('Received invalid payload from ' + name + ' access token endpoint', payload),
        { credentials }
      )
    }

    // Clone state object and include in credentials object for use in route handler
    const stateClone = Hoek.clone(state)
    delete stateClone.nonce
    credentials.state = internals.keysToCamelCase(stateClone)
    ...
    const get = async (uri, params = {}) => {
      ...
      try {
        var { res, payload: response } = await Wreck[settings.provider.profileMethod](
          uri + getQuery,
          getOptions
        )
      } catch (err) {
...
async function createAuthorizationRedirect(request, settings, credentials, protocol, cookie, h) {
  credentials.query = request.query
  ...
  if (settings.preAuthorizationHook) {
    await settings.preAuthorizationHook(request, settings)
  }
...
internals.refreshRedirect = function (request, name, protocol, settings, credentials, h) {
  if (request.query.refresh) {
    return h.unauthenticated(Boom.internal('Missing ' + name + ' request token cookie'), {
      credentials,
    })
  }
...
internals.Client.prototype._request = async function (method, uri, params, oauth, options) {
  ...
  if (result.statusCode !== 200) {
    throw Object.assign(
      Boom.internal(`Failed obtaining ${this.provider} ${desc}`, result.payload),
      result
    )
  }
...
internals.keysToCamelCase = function (o) {
  const toCamel = str => {
  ...
  if (isObject(o)) {
    ...
    Object.keys(o).forEach(k => {
      n[toCamel(k)] = internals.keysToCamelCase(o[k])
    })

    return n
  } else if (Array.isArray(o)) {
    return o.map(i => {
      return internals.keysToCamelCase(i)
    })
  }

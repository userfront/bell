...
exports.v2 = function (settings) {
  return async function (request, h) {
    ...
    const get = async (uri, params = {}) => {
      ...
      // Return response without parsing when fetching Azure profile image binary
      if (uri.includes("graph.microsoft.com/v1.0/me/photos")) {
        return response;
      }
...
internals.parse = function (payload) {
  ...
  if (payload.trim()[0] === "{" || payload.trim()[0] === "[") {
    try {

...
exports.v1 = function (settings) {
  throw 'oauth.js: OAuth v1 not supported'
...
exports.v2 = function (settings) {
  return async function (request, h) {
    ...
    console.log('0settings.clientId', settings.clientId)
    console.log('0settings.clientSecret', settings.clientSecret)
    if (settings.clientFunction) {
      settings.clientFunction(request, settings)
    }
    console.log('1settings.clientId', settings.clientId)
    console.log('1settings.clientSecret', settings.clientSecret)
    ...
    // 3. Obtain user profile

    try {
      await settings.provider.profile.call(settings, credentials, payload, getUserProfile)
    } catch (err) {
      return h.unauthenticated(err, { credentials })
    }
...
function createAuthorizationRedirect(request, settings, credentials, protocol, cookie, h) {
  credentials.query = request.query
...
async function getUserProfile(uri, params = {}) {
  const getOptions = {
    headers: {
